...
)
{
	...
	unsigned int const maxChunkSize = 1000;
	for (unsigned int chunkSize = 0; _iterator != _end && !SemanticInformation::breaksCSEAnalysisBlock(*_iterator, _msizeImportant) && chunkSize < maxChunkSize; ++_iterator, ++chunkSize)
		feedItem(*_iterator);
	if (_iterator != _end)
